<div class="row" id="principal_modal">
    <div class="col-md-12">
        <div class="modal-header">
            <h5 class="modal-title"><i class="fas fa-bus"></i>  {{__('Create User')}}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        {{ Form::open(['route'=>'users.store','id'=>'frmUsers', 'class'=>'form', 'autocomplete'=>'Off']) }}
        <div class="modal-body">
            <div class="row m-3">
                <div class="col-xs-12 col-sm-12 col-md-12 mb-4 p-0">
                    <div class='form-group floating-label'>
                        {{ Form::text('name', null, ['class' => 'form-control email', 'id'=> 'name', 'placeholder' => __('User Email')])}}
...
$(document).ready(function (){
    $('#save').on('click', function(){
        $.post("storeUsers", $('#frmUsers').serialize(), function(data){
            if(data.status == 1){
